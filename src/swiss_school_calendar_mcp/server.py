...
from .constants import CANTON_CODES, NAGER_BASE, OPENHOLIDAYS_BASE, SCHOOL_TYPE_SUFFIX
from .models import (
    Canton,
    CantonListResponse,
    DateCheckResponse,
...
mcp = FastMCP("swiss-school-calendar-mcp")
...
def _degraded(exc: Exception, attribution: str) -> dict[str, Any]:
    return {
        "source": attribution,
        "provenance": "degraded",
        "retrieved_at": utc_now_iso(),
        "note": (
            f"Upstream is currently unavailable ({exc}). No cached copy for this query. "
...
async def list_cantons(language: str = "DE") -> CantonListResponse:
    ...
    async with HolidayClient() as client:
        try:
            ...
            return CantonListResponse(**_degraded(exc, _OH), cantons=[])
...
async def list_school_types(
    canton: str | None = None, language: str = "DE"
) -> SchoolTypeListResponse:
    ...
    async with HolidayClient() as client:
        try:
            ...
            return SchoolTypeListResponse(**_degraded(exc, _OH), school_types=[])
...
async def get_school_holidays(
    ...
    async with HolidayClient() as client:
        try:
            raw, provenance, stamp = await client.school_holidays(
                valid_from, valid_to, lang, canton.upper()
            )
        except UpstreamError as exc:
            return HolidayListResponse(**_degraded(exc, _OH), count=0, holidays=[])
    ...
    if not periods:
        note = (
            f"No school holidays returned for {canton} between {valid_from} and {valid_to}. "
    ...
    async with HolidayClient() as client:
        try:
            raw, provenance, stamp = await client.public_holidays(
                f"{year}-01-01", f"{year}-12-31", lang, canton.upper()
            )
        except UpstreamError as exc:
            return HolidayListResponse(**_degraded(exc, _OH), count=0, holidays=[])
...
async def check_date(
    check_date_iso: str, canton: str, school_type: str | None = None, language: str = "DE"
) -> DateCheckResponse:
    ...
    async with HolidayClient() as client:
        try:
            school_raw, provenance, stamp = await client.school_holidays(
                window_from, window_to, lang, canton.upper()
            )
            public_raw, _, _ = await client.public_holidays(
                window_from, window_to, lang, canton.upper()
            )
        except UpstreamError as exc:
...
async def compare_school_holidays(
    cantons: list[str], year: int, school_type: str | None = "VS", language: str = "DE"
) -> OverlapResponse:
    ...
    async with HolidayClient() as client:
        try:
            ...
            return OverlapResponse(
                **_degraded(exc, _OH), year=year, school_type_filter=school_type, rows=[]
            )
...
async def find_common_free_window(
    ...
    async with HolidayClient() as client:
        try:
            ...
            return WindowResponse(**_degraded(exc, _OH), windows=[])
...
async def next_school_holidays(
    canton: str, count: int = 3, school_type: str | None = "VS", language: str = "DE"
) -> HolidayListResponse:
    ...
    async with HolidayClient() as client:
        try:
            raw, provenance, stamp = await client.school_holidays(
                today.isoformat(), (today + timedelta(days=420)).isoformat(), lang, canton.upper()
            )
        except UpstreamError as exc:
            return HolidayListResponse(**_degraded(exc, _OH), count=0, holidays=[])
...
@mcp.tool(annotations={"readOnlyHint": True})
async def get_long_weekends(year: int) -> LongWeekendResponse:
    ...
    async with HolidayClient() as client:
        try:
            ...
            return LongWeekendResponse(**_degraded(exc, _NG), year=year, long_weekends=[])
